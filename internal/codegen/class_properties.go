...
func generatePropertyAssignment(member *ast.MemberExpr, operator string, value ast.Expression) (goast.Stmt, error) {
	...
	if operator == "" || operator == "=" {
		return &goast.ExprStmt{X: propertyCall(receiver, member.PropertySetter, rhs)}, nil
	}
	...
	statements := []goast.Stmt{
		...
		&goast.AssignStmt{Lhs: []goast.Expr{current}, Tok: token.DEFINE, Rhs: []goast.Expr{propertyCall(object, member.PropertyGetter)}},
	}
	if operator == "++" || operator == "--" {
		op := token.INC
		if operator == "--" {
	...
	statements = append(statements, &goast.ExprStmt{X: propertyCall(object, member.PropertySetter, current)})
