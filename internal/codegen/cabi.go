...
// header for explicitly exported Kinmokusei functions. Semantic checking must
// run first; this function still rejects unsupported types defensively.
func GenerateCABI(program *kinmokuseiAST.Program) (CABIOutput, error) {
	...
	cHeader.WriteString("#ifndef KINMOKUSEI_GENERATED_ABI_H\n#define KINMOKUSEI_GENERATED_ABI_H\n\n#include <stdint.h>\n\n")
	cHeader.WriteString("#define KINMOKUSEI_ABI_OK ((int32_t)0)\n")
	cHeader.WriteString("#define KINMOKUSEI_ABI_PANIC ((int32_t)1)\n")
	cHeader.WriteString("#define KINMOKUSEI_ABI_INVALID_ARGUMENT ((int32_t)2)\n")
	fmt.Fprintf(&cHeader, "#define KINMOKUSEI_ABI_GATEWAY_VERSION_MAJOR %d\n", cabiGatewayVersionMajor)
	fmt.Fprintf(&cHeader, "#define KINMOKUSEI_ABI_GATEWAY_VERSION_MINOR %d\n", cabiGatewayVersionMinor)
	fmt.Fprintf(&cHeader, "#define KINMOKUSEI_ABI_FINGERPRINT %q\n\n", fingerprint)
	...
	cHeader.WriteString("#ifdef __cplusplus\n}\n#endif\n\n#endif /* KINMOKUSEI_GENERATED_ABI_H */\n")
...
func generateCABIManifest(program *kinmokuseiAST.Program, exports []kinmokuseiAST.CABIExport) ([]byte, string, error) {
...
func hasTopLevelFunction(program *kinmokuseiAST.Program, name string) bool {
	for _, declaration := range program.Declarations {
		function, ok := declaration.(*kinmokuseiAST.FunctionDecl)
...
func cabiExports(program *kinmokuseiAST.Program) []kinmokuseiAST.CABIExport {
	exports := append([]kinmokuseiAST.CABIExport(nil), program.CABIExports...)
	sort.Slice(exports, func(left, right int) bool { return exports[left].Symbol < exports[right].Symbol })
	return exports
}

func generateCABIWrapper(goSource, cHeader *strings.Builder, program *kinmokuseiAST.Program, export kinmokuseiAST.CABIExport) error {
...
func mapCABIType(ref kinmokuseiAST.TypeRef, allowVoid bool) (cabiScalar, bool) {
...
func mapProgramCABIType(program *kinmokuseiAST.Program, ref kinmokuseiAST.TypeRef, allowVoid bool) (cabiScalar, bool) {
	...
	for _, declaration := range program.Declarations {
		enumeration, ok := declaration.(*kinmokuseiAST.EnumDecl)
...
func mapCABIIntegerUnderlying(program *kinmokuseiAST.Program, ref kinmokuseiAST.TypeRef, visiting map[string]bool) (cabiScalar, bool) {
	if ref.Nullable || ref.Qualifier != "" || len(ref.GenericArguments) != 0 || ref.IsArray() || ref.IsPointer() || ref.IsFunction() || ref.IsObject() {
		return cabiScalar{}, false
	}
	switch ref.Name {
