...
func TestConstraintMethodComposition(t *testing.T) {
	...
	for _, test := range []struct{ name, source, want string }{
		...
		{"union cannot erase methods", `import go fmt from "fmt"; constraint A=~int&fmt.Stringer; constraint B=A|string;`, "union operands must be type sets without method requirements"},
		...
		{"imported type set", `import go cmp from "cmp"; constraint A=cmp.Ordered&~int;`, "imported constraint operands must be ordinary Go interfaces"},
