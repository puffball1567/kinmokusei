...
func TestGenericMethodSemanticFailureMatrix(t *testing.T) {
	for _, test := range []struct{ name, source, want string }{
		...
		{"inconsistent inference", `class Pair { public function choose<T>(left: T, right: T): T { return left; } } function use(pair: Pair): int { return pair.choose(1, "bad"); }`, `cannot use string as int`},
