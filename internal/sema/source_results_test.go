...
func TestMultipleResultBoundaries(t *testing.T) {
	for _, tc := range []struct{ name, source, want string }{
		...
		{"inferred nil", `const f=()=>{return 1,nil;};`, "cannot infer multiple result 2"},
		{"inferred count", `const f=(flag:boolean)=>{if(flag){return 1,2;}return 1,2,3;};`, "multiple result count mismatch"},
		{"inferred type", `const f=(flag:boolean)=>{if(flag){return 1,2;}return 1,"bad";};`, "cannot use"},
		{"inferred bare", `const f=(flag:boolean)=>{if(flag){return 1,2;}return;};`, "cannot mix bare"},
		{"inferred nullable", `const f=()=>{return 1,null;};`, "cannot infer multiple result 2"},
		{"inferred nested result", `function pair():(int,int){return 1,2;}const f=()=>{return 1,pair();};`, "require destructuring"},
		{"inferred narrow overflow", `const f=(flag:boolean)=>{if(flag){return byte(1),2;}return 256,2;};`, "cannot"},
