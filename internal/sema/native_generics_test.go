...
func TestNativeGenericFunctionFailureMatrix(t *testing.T) {
	tests := []struct {
		...
		{"inconsistent inference", `function choose<T>(left: T, right: T): T { return left; } function bad(): int { return choose(1, "x"); }`, `cannot use string as int`},
		{"explicit argument mismatch", `function identity<T>(value: T): T { return value; } function bad(): int { return identity<int>("x"); }`, `cannot use string as int`},
