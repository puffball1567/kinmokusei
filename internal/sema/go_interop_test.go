...
func TestUnsafeGoBuiltinFailureMatrix(t *testing.T) {
	tests := []struct {
		...
		{"offsetof non-selector", `function value(): int { return int(danger.Offsetof(1)); }`, "requires a struct field selector"},
		{"offsetof method", `import go clock from "time"; function value(input: clock.Time): int { return int(danger.Offsetof(input.Unix)); }`, "requires a struct field selector"},
