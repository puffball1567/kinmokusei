...
func TestClassFieldInitializerSemanticMatrix(t *testing.T) {
	for _, test := range []struct{ name, input, want string }{
		...
		{"earlier initialized field", `class Box { public first: int = 2; public second: int = this.first + 3; }`, ""},
		{"earlier generic field", `class Box<T> { public values: T[] = []; public count: int = len(this.values); }`, ""},
		{"earlier reference field", `class Leaf {} class Box { public first: Leaf = new Leaf(); public second: Leaf = this.first; }`, ""},
		{"inherited public field", `class Base { public value: int = 3; } class Child extends Base { public next: int = this.value + 1; }`, ""},
		{"inherited protected field", `class Base { protected value: int = 3; } class Child extends Base { public next: int = this.value + 1; }`, ""},
		{"inherited constructor field", `class Leaf {} class Base { public leaf: Leaf; constructor() { this.leaf = new Leaf(); } } class Child extends Base { public next: Leaf = this.leaf; }`, ""},
		{"inherited generic field", `class Base<T> { public value: T[] = []; } class Child<U> extends Base<U[]> { public size: int = len(this.value); }`, ""},
		...
		{"inherited private field", `class Base { private value: int = 3; } class Child extends Base { public next: int = this.value; }`, "private"},
		{"captured inherited field", `class Base { public value: int = 3; } class Child extends Base { public callback: () => int = (): int => this.value; }`, "instance field initializers may read only earlier initialized fields"},
