...
func TestBitwiseAndShiftFailureMatrix(t *testing.T) {
	tests := []struct {
		...
		{"and-not float right", `function value(input: int): int { return input &^ 1.5; }`, "truncated to int"},
		{"shift float left", `function value(input: float): int { return input << 1; }`, "operator << requires integer operands"},
		{"shift float right", `function value(input: int): int { return input >> 1.5; }`, "truncated to uint"},
