...
func TestCollectionBuiltinSemanticFailureMatrix(t *testing.T) {
	tests := []struct {
		...
		{"max mixed types", `function bad(value: int): int { return max(value, "x"); }`, "mismatched types"},
		{"min distinct typed numbers", `function bad(left: int, right: int64): int { return min(left, right); }`, "mismatched types"},
		{"max integer overflow", `function bad(value: byte): byte { return max(value, 256); }`, "overflows uint8"},
