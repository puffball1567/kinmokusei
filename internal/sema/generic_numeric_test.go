...
func TestGenericNumericConstantArguments(t *testing.T) {
	...
	for _, test := range []struct{ name, source, want string }{
		...
		{"native constant alias", `const n=255;const alias=n;function keep<T>(v:T):T{return v;}function f():byte{return keep<byte>(alias);}`, ""},
		{"native runtime alias", `const n=255;let alias=n;function keep<T>(v:T):T{return v;}function f():byte{return keep<byte>(alias);}`, "cannot"},
