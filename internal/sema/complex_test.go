...
func TestComplexSemanticMatrix(t *testing.T) {
	for _, test := range []struct{ name, source, want string }{
		...
		{"nonconstant shift construction", `function f(n:uint):complex128{return complex(1<<n,0);}`, "must be integer"},
		{"nonconstant shift projection", `function f(n:uint):float{return real(1<<n);}`, "must be integer"},
