...
| 10 | Source-declared multiple results | Implemented for `(T, U)` signatures on functions, methods, arrows and function types, explicit comma-separated returns, call forwarding, contextual callbacks and try/catch/finally; inference without context and named results remain |
...
| Source-declared multiple results | Source functions, methods, arrows and function types accept `(T, U)` signatures, explicit comma-separated returns and forwarding, including try/catch/finally; explicit values support contextual checks and class upcasts, while forwarding requires direct Go assignability; generic aliases and nullable callback contracts are checked | Result-list inference without context, named results and multiple-result properties are not supported |
