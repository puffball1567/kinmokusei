...
| 10 | Source-declared multiple results | Partially implemented: `(T, U)` signatures on functions, methods, arrows and function types; direct call forwarding and contextual callbacks supported; explicit comma-separated returns and exception lowering remain |
...
| Source-declared multiple results | Source functions, methods, arrows and function types accept `(T, U)` result lists and direct forwarding returns; result elements must be ordinary values and directly assignable in generated Go; generic aliases and nullable callback contracts are checked | Add comma-separated returns and exception lowering; result-list inference without context, named results and multiple-result properties are not supported |
