...
|---|---|---|
| Import alias name isolation | An import renamed through a re-export can bind to an unrelated local whose name matches the dependency's original declaration after source linking | Make linked declaration names hygienic with respect to local bindings and type parameters; preserve public Go names and editor declaration identities |
