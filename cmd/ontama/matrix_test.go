...
func TestCommandFailureMatrix(t *testing.T) {
	tests := []struct {
		...
		{"lsp source argument", []string{"lsp", "--stdio", "main.otm"}, 2, "accepts no source arguments"},
...
func TestGoInteropAuditCommandMatrix(t *testing.T) {
	...
	missing := "example.invalid/ontama-audit-missing"
...
func TestCheckAndEmitGoMatrix(t *testing.T) {
	temp := t.TempDir()
	valid := filepath.Join(temp, "valid.otm")
	invalid := filepath.Join(temp, "invalid.otm")
	...
	if status, _, stderr := captureRun(t, "check", filepath.Join(temp, "missing.otm")); status != 1 || stderr == "" {
...
func TestCheckJSONDiagnosticMatrix(t *testing.T) {
	temp := t.TempDir()
	valid := filepath.Join(temp, "valid.otm")
	additional := filepath.Join(temp, "additional.otm")
	lexical := filepath.Join(temp, "lexical.otm")
	semantic := filepath.Join(temp, "semantic.otm")
	syntax := filepath.Join(temp, "syntax.otm")
	library := filepath.Join(temp, "library.otm")
	...
	missing := filepath.Join(temp, "missing.otm")
...
func TestEmitCABIMatrix(t *testing.T) {
	...
	valid := filepath.Join(temp, "valid.otm")
	invalid := filepath.Join(temp, "invalid.otm")
	if err := os.WriteFile(valid, []byte(`export c("ontama_answer") function answer(): int32 { return 42; }`), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(invalid, []byte(`export c("ontama_answer") function answer(value: string): void {}`), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	for name, want := range map[string]string{
		"generated.go": "func answer() int32", "generated_cabi.go": "//export ontama_answer", "ontama_abi.h": "int32_t ontama_answer",
		"ontama_abi.json": `"symbol": "ontama_answer"`,
	} {
...
func TestABICompatibilityCommandMatrix(t *testing.T) {
	...
	additive := filepath.Join(temp, "additive.otm")
	breaking := filepath.Join(temp, "breaking.otm")
	for path, source := range map[string]string{
		base:     `export c("ontama_value") function value(input: int32): int32 { return input; }`,
		additive: `export c("ontama_value") function value(input: int32): int32 { return input; } export c("ontama_ping") function ping(): void {}`,
		breaking: `export c("ontama_value") function value(input: int64): int32 { return int32(input); }`,
	} {
		if err := os.WriteFile(path, []byte(source), 0o644); err != nil {
			t.Fatal(err)
		}
	...
	manifest := filepath.Join(output, "ontama_abi.json")
	...
	if status, stdout, stderr := captureRun(t, "abi", "check", "--baseline", manifest, additive); status != 0 || !strings.Contains(stdout, "added symbols ontama_ping") || stderr != "" {
		t.Fatalf("additive: status=%d stdout=%q stderr=%q", status, stdout, stderr)
	}
	if status, stdout, stderr := captureRun(t, "abi", "check", "--baseline", manifest, breaking); status != 1 || stdout != "" || !strings.Contains(stderr, `changed signature of C ABI symbol "ontama_value"`) {
...
func TestBuildAndRunPropagateGoFailures(t *testing.T) {
	temp := t.TempDir()
	t.Setenv("GOCACHE", filepath.Join(temp, "go-cache"))
	source := filepath.Join(temp, "library.otm")
