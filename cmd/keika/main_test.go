...
func TestInstallationGuideHelloExampleChecks(t *testing.T) {
	source := filepath.Join("..", "..", "examples", "hello", "main.km")
	if status := run([]string{"check", source}); status != 0 {
		t.Fatalf("installation guide example check exit status = %d", status)
	}
}

func TestLegacySourceExtensionRemainsCheckableDuringMigration(t *testing.T) {
	source := filepath.Join(t.TempDir(), "legacy.otm")
	if err := os.WriteFile(source, []byte(`function main(): void {}`), 0o644); err != nil {
		t.Fatal(err)
	}
	if status := run([]string{"check", source}); status != 0 {
		t.Fatalf("legacy source check exit status = %d", status)
	}
}
