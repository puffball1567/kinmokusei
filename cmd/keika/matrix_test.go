...
func TestCommandFailureMatrix(t *testing.T) {
	tests := []struct {
		...
		{"deps update all without project", []string{"deps", "update"}, 1, "cannot read project manifest"},
		{"deps update latest without project", []string{"deps", "update", "example.com/library"}, 1, "cannot read project manifest"},
