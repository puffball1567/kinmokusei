...
import (
	...
	"github.com/puffball1567/kinmokusei/internal/lsp"
...
func runInstall(args []string) int {
	...
	if !*goModule {
		fmt.Fprintln(os.Stderr, "install currently requires --go-module; Kinmokusei source package installation is not implemented")
		return 2
	}
...
func runEmitCABI(args []string) int {
	...
	flags.SetOutput(os.Stderr)
	output := flags.String("o", "", "write generated.go, generated_cabi.go, kinmokusei_abi.h, and kinmokusei_abi.json to this directory")
	if err := flags.Parse(args); err != nil {
		return 2
	}
	if *output == "" {
	...
	for name, contents := range map[string][]byte{
		...
		"kinmokusei_abi.h": artifacts.Header, "kinmokusei_abi.json": artifacts.Manifest,
	} {
...
func runABI(args []string) int {
	...
	flags.SetOutput(os.Stderr)
	baselinePath := flags.String("baseline", "", "compare against this previously generated kinmokusei_abi.json")
	if err := flags.Parse(args[1:]); err != nil {
		return 2
	}
	if *baselinePath == "" {
		fmt.Fprintln(os.Stderr, "abi check requires --baseline with a kinmokusei_abi.json file")
		return 2
	}
	if flags.NArg() == 0 {
