...
pronounced roughly "keen-moh-koo-say") is a programming language for writing
web backends and Go libraries with TypeScript-inspired syntax and compiling
them to readable Go source. The CLI is named `keika`, and source files use the
`.km` extension.

The name is the Japanese word for sweet osmanthus, an evergreen known for its
small, fragrant autumn flowers. The shorter CLI name **keika** comes from 桂花,
another name for osmanthus.

## Public identity

| Use | Name |
| --- | --- |
| Formal language name | Kinmokusei Programming Language |
| Normal display name | Kinmokusei |
| CLI and executable | `keika` |
| Go command path | `cmd/keika` |
| CLI release archive | `keika_<version>_<os>_<arch>.*` |
| Source extension | `.km` |
| Compiler-managed state | `.kinmokusei/` |
| Project metadata | `kinmokusei.toml`, `kinmokusei.lock` |
| Standard-library namespace | `kinmokusei/http` |
| Generated C ABI namespace | `kinmokusei_*`, `KINMOKUSEI_ABI_*` |
| VS Code language ID and VSIX | `kinmokusei` |
| Language-server command | `keika lsp --stdio` |

Kinmokusei is a TypeScript-inspired source dialect for the Go ecosystem. It is
...
[v0.1 migration guide](docs/migrating-from-v0.1.md).
...
- Keep emitted Go standalone and suitable for ordinary Go repository publication; authors may distribute `.km`, generated Go, or both.
...
- Allow Kinmokusei packages to import one another.
- Use `import go` as an explicit low-level escape hatch into the Go ecosystem.
- Build safer, idiomatic Kinmokusei libraries on top of direct Go interop.
- Keep the compiler and `keika lsp --stdio` in the same versioned distribution.
...
- An 82/82 implemented Go-equivalent runtime contract registry backed by isolated handwritten-Go differential tests; new accepted runtime features must extend the registry and oracle together.
...
Unsupported APIs and operations are rejected at their Kinmokusei use site instead of being silently passed through to generated Go.
...
Install a released `keika` binary and matching Visual Studio Code extension
using the [installation guide](docs/installation.md). Developers with Go
installed can instead install the latest tagged compiler with:
...
```sh
go run ./cmd/keika check examples/basic/main.km
go run ./cmd/keika check --json examples/basic/main.km
go run ./cmd/keika emit-go examples/basic/main.km
go run ./cmd/keika emit-c-abi -o ./generated-c-abi examples/c-abi/library.km
go run ./cmd/keika abi check --baseline ./previous/kinmokusei_abi.json examples/c-abi/library.km
go run ./cmd/keika interop audit --stdlib
go test ./...
./scripts/coverage.sh
```
...
backends written in Kinmokusei. Both backends use the real external Go
...
Kinmokusei's [quality and Go compatibility policy](docs/quality-and-go-compatibility.md)
...
with. When installing from source, build `keika` with the newest Go version it
will target. A future Go minor release may require a corresponding Kinmokusei
