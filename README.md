...
- `boolean`, `string`, signed integers, fixed-width `uint16`/`uint32`/`uint64`, floating-point types, and `void`.
- Identical `number`, `float`, and `float64` types.
- Typed functions, top-level generic functions, native generic structs, arrow functions, function types, and local type inference. Native generic calls support inference plus TypeScript-shaped `<T>` and Go-shaped `[T]` explicit or partial type arguments; generic struct instantiation is explicit.
- Reference-type classes, constructors, `this`, `new`, public/private/protected visibility, instance/static methods, idiomatic public static Go APIs without private/protected leakage, and explicit single inheritance with `extends`, `virtual`, `override`, `final`, `super`, multi-level dispatch, nil/identity-preserving implicit upcasts, checked/forced class downcasts, and public Go hierarchy conversion APIs.
- Nominal value-type `struct` declarations, including unconstrained generic structs, with complete named literals, Go-compatible assignment/argument/return copying, explicit pointer sharing, nested value/pointer receiver methods, method values, shallow reference-field copying, comparability, recursive indirection, and relative-module linking. Non-generic structs also support external receiver declarations.
- Native nominal defined types with `type Name = distinct T`, transparent aliases with `alias Name = T`, explicit Go-compatible conversions, untyped-constant assignment, collection/operator support, and relative-module linking.
...
- `while`, C-style `for`, collection/channel `for-of` range, `break`, and `continue`.
...
- Local destructuring of multiple results, blank bindings, and multiple-result reassignment.
...
Not yet implemented include generic classes/interfaces/defined-type declarations, external receiver declarations for generic structs, constrained native type parameters, methods on native defined types, recursive native type declarations, distinct definitions over native classes/structs/interfaces, general OnsenTamago package distribution, explicit SPDX configuration, editor marketplace publication, remaining general retained/static FFI data ownership policies, broader constructor cardinality proofs for unknown collections, automatic task cancellation/context inheritance, and the high-level web framework above the implemented HTTP routing kernel. Stable class-member nullable flow is implemented with conservative invalidation across aliases, writes, addresses, closures, and unknown calls. Boolean/integer/string constant expressions and local, `for`-initializer, same-file global, or explicitly imported `const` chains can prove guaranteed constructor-loop entry; provably nonempty array/string/fixed-array/`append`/`makeSlice` ranges participate in the same check. A JSON API using direct `net/http` and `encoding/json` interop already compiles and runs.
