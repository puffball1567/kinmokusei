# Kinmokusei for Visual Studio Code

This directory contains the official thin Visual Studio Code client for
Kinmokusei. It recognizes `.km` files, provides syntax highlighting, and
...
provided by the versioned language server distributed with the `keika` CLI.

## Configuration

`kinmokusei.server.path` selects the `keika` executable. The default resolves
`keika` from the editor process's `PATH`. After changing the setting, the
extension serially stops the old server and starts the configured executable.
Use **Kinmokusei: Restart Language Server** to retry a failed startup or restart
...
The end-to-end test builds the current `keika` command, starts a real Extension
...
The package command writes `dist/kinmokusei-<version>.vsix`. It creates the
...
attached to each GitHub release with the `keika` binaries; marketplace
publication remains a separate release decision.

For interactive development, open this directory in Visual Studio Code and
start an Extension Development Host. The `keika` executable must be on `PATH`
or configured through `kinmokusei.server.path`.

Other LSP-capable editors do not need this extension; configure them to launch
`keika lsp --stdio` for `.km` files.
